...
{colorize_text("Fuel the descent: https://buymeacoffee.com/amariahak", "yellow")}
    ...
    def help_game_flow(self):
        ...
        print(f"""
{colorize_text('Exploration System:', 'yellow')}
• Press 'E' to explore and progress through chapters
• Each exploration reveals different encounters:
  - Safe: NPCs, loot caches, rest areas, lore fragments
  - Combat: Mobs with options to Fight/Flee/Prepare
  - Shop: Merchants appear, use 'T' to trade
  - Miniboss: Elite enemies with enhanced stats
  - Boss: Floor guardians, no escape possible

{colorize_text('Chapter Blueprint:', 'yellow')}
• Entity generates unique 9-chapter sequence each run
• Sequence changes every death—no repetition
• Adaptive content based on your playstyle patterns

{colorize_text('Safe vs Hostile Areas:', 'yellow')}
• Safe areas: No timer, can type 'exit' to end gracefully
• Hostile areas: Entity denies escape
• Combat encounters make areas temporarily hostile

{colorize_text('Progression:', 'yellow')}  
• Complete all 9 chapters to advance to next floor
• Floors 1-5 each have unique themes and challenges
• Read lore calmly between encounters
        """)
...
• END: Stamina, special abilities → {colorize_text('Stamina-draining attacks increase', 'red')}
• VIT: HP + healing power → {colorize_text('BUT healing becomes more expensive!', 'red')}

{colorize_text('Souls-Like Scaling:', 'yellow')}
• Stat upgrade costs: 10 → 15 → 35 → 85+ Ashlight  
• Healing costs scale with VIT: 8 → 28 → 68+ Ashlight
• Higher VIT = more health + better healing BUT much more expensive
• Healing never restores full HP—always capped at 60%

{colorize_text('The VIT Trap:', 'red')}
{colorize_text('High VIT gives massive health but makes healing unaffordable.', context='lore')}
...
{colorize_text('The Entity is always watching. Vary your patterns, or become data.', context='lore')}
    ...
    def show_main_menu(self):
        ...
        location_desc = f"\n{colorize_text(f'Book {self.player.floor}', 'red')} - {self.get_current_location_desc()}"
    ...
    def get_current_location_desc(self) -> str:
        ...
        return book_descs.get(self.player.floor, "Unknown depths...")
    ...
    def process_action(self, action: str):
        ...
        if action == 'explore':  # New unified exploration system
            self.explore_and_progress()
            
        elif action == 'a':  # Attack (combat only)
            if self.in_combat:
                self.combat.player_attack(self.player)
                self.in_dangerous_area = True  # Mark as dangerous after combat
            else:
                print(f"{colorize_text('Nothing to attack here. Try exploring instead.', 'yellow')}")
                
        elif action == 'd':  # Dodge (combat only)
            if self.in_combat:
                self.combat.player_dodge(self.player)
            else:
                print(f"{colorize_text('Nothing to dodge. Try exploring instead.', 'yellow')}")
    ...
    def use_healing_item(self):
        """Use healing item or ability - Souls-like scaling with VIT"""
        
        # Healing cost increases with VIT (Souls-like scaling)
        vit_stat = self.player.stats["vit"]
        if vit_stat <= 10:
            heal_cost = 8  # Base cost for low VIT
        elif vit_stat <= 20:
            heal_cost = 8 + (vit_stat - 10) * 2  # +2 per point 11-20
        elif vit_stat <= 30:
            heal_cost = 28 + (vit_stat - 20) * 4  # +4 per point 21-30
        else:
            heal_cost = 68 + (vit_stat - 30) * 6  # +6 per point 30+
        
        # Healing amount scales with VIT but stays consistent (like fixed amount per VIT)
        heal_amount = 25 + (vit_stat - 10) * 2  # 25 base + 2 per VIT above 10
        
        # Cap healing at 60% of max HP (never full heal - Souls-like)
        max_heal_percent = 0.6
        heal_amount = min(heal_amount, int(self.player.max_health * max_heal_percent))
        
        print(f"\n{colorize_text('═══ SOULS-LIKE HEALING ═══', 'cyan')}")
        print(f"Current Health: {colorize_text(f'{self.player.health}/{self.player.max_health}', 'white')}")
        print(f"Heal Amount: {colorize_text(f'{heal_amount} HP', 'green')} (VIT {vit_stat})")
        print(f"Heal Cost: {colorize_text(f'{heal_cost} Ashlight', 'yellow')} (scales with VIT)")
        print(f"Max Healing: {colorize_text(f'{int(self.player.max_health * max_heal_percent)} HP', 'cyan')} (60% cap)")
        
        if self.player.health >= self.player.max_health:
            print(f"\n{colorize_text('You are already at full health!', 'green')}")
        elif self.player.ashlight >= heal_cost:
            old_health = self.player.health
            self.player.ashlight -= heal_cost
            self.player.heal(heal_amount)
            actual_heal = self.player.health - old_health
            print(f"\n{colorize_text(f'Healed for {actual_heal} HP ({old_health} → {self.player.health})', 'green')}")
            print(f"Ashlight remaining: {colorize_text(str(self.player.ashlight), 'yellow')}")
            print(f"{colorize_text('Note: Higher VIT = more healing but higher cost', 'yellow')}")
        else:
            print(f"{colorize_text(f'Not enough Ashlight to heal. Need {heal_cost}, have {self.player.ashlight}.', 'red')}")
            print(f"{colorize_text('Warning: VIT scaling makes healing expensive at high levels', 'red')}")
    ...
    def show_inventory(self):
        """Show interactive inventory with equipment visualization"""
        while True:
            clear_screen()
            print(f"{colorize_text('═══ INVENTORY & EQUIPMENT ═══', 'cyan')}")
            
            # Tab selection
            print(f"\n{colorize_text('Tabs:', 'yellow')}")
            print(f"  1. {colorize_text('BAG', 'cyan')} - All items ({len(self.player.inventory)} items)")
            print(f"  2. {colorize_text('EQUIPPED', 'green')} - Current equipment")
            print(f"  0. {colorize_text('Return to game', 'white')}")
            
            try:
                tab_choice = input(f"\n{colorize_text('Choose tab (0-2):', 'white')} ").strip()
    ...
    def show_bag_tab(self):
        """Show bag tab with all items"""
        clear_screen()
        print(f"{colorize_text('═══ BAG - ALL ITEMS ═══', 'cyan')}")
        
        if not self.player.inventory:
            print(f"{colorize_text('Your bag is empty.', 'white')}")
        ...
        for i, item in enumerate(self.player.inventory):
            equipped_marker = "⚡" if self.is_item_equipped(item) else "  "
            print(f"  {equipped_marker}{i+1}. {colorize_text(item['name'], 'yellow')}")
        print(f"  0. Return to inventory tabs")
    ...
    def show_equipped_tab(self):
        """Show equipped tab with equipment silhouette"""
        clear_screen()
        print(f"{colorize_text('═══ EQUIPPED GEAR ═══', 'green')}")
        
        # Equipment silhouette
        print(f"\n{colorize_text('Equipment Silhouette:', 'yellow')}")
        print(f"""
        ┌─────────────┐
        │    HEAD     │ {self.get_equipped_item_name('head')}
        │             │
        │   ┌─────┐   │
        │   │CHEST│   │ {self.get_equipped_item_name('chest')}
        │   └─────┘   │
        │ ARMS   ARMS │ {self.get_equipped_item_name('arms')}
        │             │
        │    LEGS     │ {self.get_equipped_item_name('legs')}
        │             │
        │    BOOTS    │ {self.get_equipped_item_name('boots')}
        └─────────────┘
        
        WEAPON: {self.get_equipped_item_name('weapon')}
        RINGS:  {self.get_equipped_item_name('rings')}
        NECK:   {self.get_equipped_item_name('necklace')}
        """)
        
        equipped_count = self.count_equipped_items()
        print(f"Equipped Items: {colorize_text(f'{equipped_count}/3', 'yellow')} buff items maximum")
        
        if equipped_count == 0:
            print(f"{colorize_text('No equipment currently equipped.', 'white')}")
            print(f"{colorize_text('Use the BAG tab to equip items.', 'cyan')}")
        
        press_enter_to_continue()
    
    def get_equipped_item_name(self, slot: str) -> str:
        """Get name of equipped item in slot"""
        # For now, simplified system
        if slot == 'weapon' and self.player.equipped_weapon:
            return colorize_text(self.player.equipped_weapon['name'], 'green')
        elif slot in ['head', 'chest', 'arms', 'legs', 'boots', 'rings', 'necklace']:
            # Placeholder for future equipment system
            return colorize_text('(Empty)', 'white')
        else:
            return colorize_text('(Empty)', 'white')
    
    def is_item_equipped(self, item: dict) -> bool:
        """Check if item is currently equipped"""
        return self.player.equipped_weapon == item
    ...
    def count_equipped_items(self) -> int:
        ...
        if self.player.equipped_weapon:
            count += 1
        # Add other equipment slots here when implemented
        return count
    ...
    def get_stat_impact_description(self, stat: str, current_value: int) -> str:
        ...
        # Calculate actual healing cost for VIT preview
        vit_preview = current_value + 1
        if vit_preview <= 10:
            next_heal_cost = 8
        elif vit_preview <= 20:
            next_heal_cost = 8 + (vit_preview - 10) * 2
        elif vit_preview <= 30:
            next_heal_cost = 28 + (vit_preview - 20) * 4
        else:
            next_heal_cost = 68 + (vit_preview - 30) * 6
        ...
        descriptions = {
            ...
            "end": f"Stamina +{5 + current_value//2}, special abilities → Drain attacks intensify",
            "vit": f"HP +{8 + current_value//3}, heal amount +4, heal cost → {next_heal_cost} Ashlight"
        }
    ...
    def explore_and_progress(self):
        """Unified exploration system that progresses through AI-generated chapters"""
        print(f"\n{colorize_text('═══ EXPLORING ═══', 'cyan')}")
        
        # Generate chapter blueprint if not exists
        if not hasattr(self, 'current_blueprint') or not self.current_blueprint:
            print(f"{colorize_text('The Entity generates your path...', 'yellow')}")
            self.current_blueprint = self.entity_ai.generate_chapter_blueprint(
                self.player.state_vector(), 
                self.player.deaths + 1
            )
            self.current_chapter = 0  # Start at chapter 0 (which is chapter 1)
            
            print(f"\n{colorize_text('Chapter Blueprint Created:', 'cyan')}")
            print(f"{colorize_text(self.current_blueprint['entity_comment'], context='lore')}")
            
            # Show the blueprint sequence
            print(f"\n{colorize_text('Your Path Through This Book:', 'yellow')}")
            for i, chapter_info in enumerate(self.current_blueprint['sequence']):
                chapter_type = chapter_info['type'].title()
                current_marker = "→" if i == 0 else " "
                print(f"  {current_marker} Chapter {i+1}: {colorize_text(chapter_type, 'cyan')}")
        
        # Progress to next chapter if we haven't finished
        if self.current_chapter < len(self.current_blueprint['sequence']):
            chapter_info = self.current_blueprint['sequence'][self.current_chapter]
            self.execute_chapter(chapter_info)
            self.current_chapter += 1  # Move to next chapter AFTER executing current
        else:
            # Completed all chapters - advance to next book
            print(f"{colorize_text('You have completed all chapters in this Book.', 'cyan')}")
            print(f"{colorize_text('The descent continues to the next Book...', 'yellow')}")
            self.player.floor += 1
            self.current_blueprint = None  # Reset for next book
            
    def execute_chapter(self, chapter_info: dict):
        """Execute a specific chapter based on AI blueprint"""
        chapter_num = chapter_info['chapter']
        chapter_type = chapter_info['type']
        corrupted = chapter_info.get('corrupted', False)
        
        print(f"\n{colorize_text(f'--- CHAPTER {chapter_num} ---', 'yellow')}")
        
        if corrupted:
            print(f"{colorize_text('⚠️  CHAOS CORRUPTION DETECTED ⚠️', 'red')}")
            corruption_type = chapter_info.get('corruption_type', 'reality_glitch')
            corruption_desc = corruption_type.replace("_", " ").title()
            print(f"{colorize_text(f'Corruption: {corruption_desc}', 'red')}")
        
        if chapter_type == "safe":
            self.execute_safe_chapter()
        elif chapter_type == "combat":
            self.execute_combat_chapter()
        elif chapter_type == "shop":
            self.execute_shop_chapter()
        elif chapter_type == "miniboss":
            self.execute_miniboss_chapter()
        elif chapter_type == "boss":
            self.execute_boss_chapter()
    
    def execute_safe_chapter(self):
        """Execute a safe chapter - NPCs, loot, rest"""
        outcomes = [
            ("npc_encounter", "You discover NPCs in this safe haven"),
            ("loot_cache", "You find a hidden cache of supplies"),
            ("rest_area", "You find a peaceful area to rest and reflect"),
            ("lore_fragment", "Ancient memories whisper in this place")
        ]
        
        outcome_type, description = random.choice(outcomes)
        print(f"{colorize_text(description, 'green')}")
        
        if outcome_type == "loot_cache":
            self.find_loot_cache()
        elif outcome_type == "rest_area":
            self.find_rest_area()
        elif outcome_type == "lore_fragment":
            self.find_lore_fragment()
        elif outcome_type == "npc_encounter":
            print(f"{colorize_text('Use \"t\" to talk to NPCs here.', 'cyan')}")
    
    def execute_combat_chapter(self):
        """Execute a combat chapter"""
        print(f"{colorize_text('The shadows stir with hostile intent...', 'red')}")
        
        # Generate mob with Entity adaptation
        mob = self.entity_ai.generate_mob(self.player.state_vector(), self.player.floor)
        
        mob_name = mob["name"]
        print(f"\n{colorize_text(f'A {mob_name} emerges from the digital darkness!', 'red')}")
        
        # Give player options before combat
        print(f"\n{colorize_text('What do you do?', 'yellow')}")
        print(f"  1. {colorize_text('Fight', 'red')} - Engage in combat")
        print(f"  2. {colorize_text('Flee', 'yellow')} - Attempt to escape")
        print(f"  3. {colorize_text('Prepare', 'cyan')} - Heal/check inventory first")
        
        try:
            choice = input(f"\n{colorize_text('Choose (1-3):', 'white')} ").strip()
            ...
            if choice == "1":
                ...
                if random.random() < 0.7:  # 70% flee success
                    ...
                    print(f"{colorize_text('Escape failed! Combat begins!', 'red')}")
                    self.start_combat(mob)
            elif choice == "3":
                print(f"{colorize_text('You prepare yourself...', 'cyan')}")
                print(f"{colorize_text('Use h/i/s to prepare, then \"explore\" to return', 'yellow')}")
                # Mob waits - player can heal/check inventory
            else:
                print(f"{colorize_text('Hesitation! The enemy attacks!', 'red')}")
                self.start_combat(mob)
                
        except (ValueError, KeyboardInterrupt):
            print(f"{colorize_text('Distraction! The enemy attacks!', 'red')}")
            self.start_combat(mob)
    
    def execute_shop_chapter(self):
        """Execute a shop chapter"""
        print(f"{colorize_text('You discover a mysterious merchant...', 'yellow')}")
        print(f"{colorize_text('Use \"t\" to talk to NPCs and trade.', 'cyan')}")
    
    def execute_miniboss_chapter(self):
        """Execute a miniboss chapter"""
        print(f"{colorize_text('A powerful presence blocks your path...', 'red')}")
        
        # Generate enhanced mob as miniboss
        miniboss = self.entity_ai.generate_mob(self.player.state_vector(), self.player.floor)
        
        # Enhance stats for miniboss
        for stat in miniboss["stats"]:
            miniboss["stats"][stat] = int(miniboss["stats"][stat] * 1.5)
        miniboss["name"] = f"Elite {miniboss['name']}"
        
        miniboss_name = miniboss["name"]
        print(f"\n{colorize_text(f'MINIBOSS: {miniboss_name} appears!', 'red')}")
        print(f"{colorize_text('This enemy looks significantly stronger...', 'yellow')}")
        
        # Same options as combat but with warning
        print(f"\n{colorize_text('What do you do?', 'yellow')}")
        print(f"  1. {colorize_text('Fight', 'red')} - Face the miniboss")
        print(f"  2. {colorize_text('Flee', 'yellow')} - Try to escape (harder)")
        print(f"  3. {colorize_text('Prepare', 'cyan')} - Heal/check inventory first")
        ...
        try:
            ...
            if choice == "1":
                self.start_combat(miniboss)
            elif choice == "2":
                if random.random() < 0.4:  # Only 40% flee success vs miniboss
                    print(f"{colorize_text('You barely escape!', 'green')}")
                    self.player.flee_encounter()
                else:
                    print(f"{colorize_text('The miniboss blocks your escape! Combat begins!', 'red')}")
                    self.start_combat(miniboss)
            elif choice == "3":
                print(f"{colorize_text('You prepare yourself for a tough fight...', 'cyan')}")
                print(f"{colorize_text('Use h/i/s to prepare, then \"explore\" to return', 'yellow')}")
            else:
                print(f"{colorize_text('The miniboss attacks while you hesitate!', 'red')}")
                self.start_combat(miniboss)
                
        except (ValueError, KeyboardInterrupt):
            print(f"{colorize_text('The miniboss strikes!', 'red')}")
            self.start_combat(miniboss)
    
    def execute_boss_chapter(self):
        """Execute a boss chapter"""
        print(f"{colorize_text('The air itself seems to thicken with malevolent power...', 'red')}")
        
        boss = self.entity_ai.generate_boss(self.player.state_vector(), self.player.floor)
        
        print(f"\n{colorize_text('💀 FLOOR BOSS ENCOUNTER 💀', 'red')}")
        print(f"{colorize_text(boss['name'].upper(), context='boss')}")
        
        # Generate boss intro lore
        boss_lore = self.entity_ai.generate_lore(
            self.player.state_vector(),
            self.player.floor,
            f"boss_encounter_{boss['name']}"
        )
        print(f"\n{colorize_text(boss_lore, context='lore')}")
        
        print(f"\n{colorize_text('There is no escape from this encounter.', 'red')}")
        print(f"  1. {colorize_text('Fight', 'red')} - Face destiny")
        print(f"  2. {colorize_text('Prepare', 'cyan')} - Final preparations")
        
        try:
            choice = input(f"\n{colorize_text('Choose (1-2):', 'white')} ").strip()
            
            if choice == "1":
                self.start_combat(boss)
            elif choice == "2":
                print(f"{colorize_text('Make your final preparations...', 'cyan')}")
                print(f"{colorize_text('Use h/i/s to prepare, then \"explore\" to face the boss', 'yellow')}")
            else:
                print(f"{colorize_text('The boss will not wait. Combat begins!', 'red')}")
                self.start_combat(boss)
                
        except (ValueError, KeyboardInterrupt):
            print(f"{colorize_text('The boss attacks!', 'red')}")
            self.start_combat(boss)
    
    def find_loot_cache(self):
        """Player finds loot cache in safe chapter"""
        item = self.entity_ai.generate_item(self.player.state_vector(), self.player.floor)
        ashlight_gain = random.randint(10, 25)
        
        self.player.inventory.append(item)
        self.player.ashlight += ashlight_gain
        
        print(f"\n{colorize_text('🎒 LOOT DISCOVERED:', 'yellow')}")
        print(f"Found: {colorize_text(item['name'], 'yellow')}")
        print(f"Gained: {colorize_text(f'{ashlight_gain} Ashlight', 'yellow')}")
    
    def find_rest_area(self):
        """Player finds rest area in safe chapter"""
        sanity_gain = random.randint(3, 8)
        stamina_gain = random.randint(5, 15)
        
        self.player.sanity = min(100, self.player.sanity + sanity_gain)
        self.player.stamina = min(self.player.max_stamina, self.player.stamina + stamina_gain)
        
        print(f"\n{colorize_text('🕯️  PEACEFUL REST:', 'green')}")
        print(f"Sanity restored: {colorize_text(f'+{sanity_gain}', 'green')}")
        print(f"Stamina restored: {colorize_text(f'+{stamina_gain}', 'green')}")
    
    def find_lore_fragment(self):
        """Player finds lore in safe chapter"""
        lore = self.entity_ai.generate_lore(
            self.player.state_vector(),
            self.player.floor,
            "lore_discovery"
        )
        
        print(f"\n{colorize_text('📜 MEMORY FRAGMENT:', context='lore')}")
        print(f"{colorize_text(lore, context='lore')}")
        
        # Lore gives small sanity boost if not too corrupted
        if self.player.sanity > 20:
            self.player.sanity = min(100, self.player.sanity + 2)
            print(f"{colorize_text('Understanding brings peace... +2 Sanity', 'green')}")
        else:
            print(f"{colorize_text('The knowledge burns your fractured mind...', 'red')}")
